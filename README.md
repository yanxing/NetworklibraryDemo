...
1.适用于返回接口json形式的项目
2.使用了Rxlifecycle2来防止RxJava的内存泄露，所以基类需要继承Rxlifecycle2相关类，或者实现LifecycleProvider接口
2.可以设置接口请求时等待对话框
3.可以设置接口请求完成后上下拉刷新控件置为完成状态，刷新组件需要实现PullToRefresh接口。
4.接口请求参数和返回数据打印。
5.可以设置自定义的拦截器、请求头参数和okHttpClient。
* gradle 
```java
